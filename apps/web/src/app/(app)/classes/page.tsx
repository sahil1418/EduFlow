        ...
        <div className="space-y-3">
          ...
          {classes.map((c) => (
            <Card key={c.id}>
              <CardHeader
                title={c.label}
                subtitle={`Grade ${c.grade} · ${c.sections.length} section${c.sections.length === 1 ? '' : 's'}`}
              />
              <CardBody>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {c.sections.map((s) => (
                    <div
                      key={s.id}
                      className="p-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)]"
                    >
                      <div className="flex items-center gap-2">
                        <span className="ef-chip ef-chip-brand">Section {s.name}</span>
                      </div>
                  ...
                  <form
                    ...
                    className="p-4 rounded-lg border border-dashed border-[var(--color-border-strong)] flex gap-2 items-end"
                  >
                    <Input
                      label="New section"
